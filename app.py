...
try:
    ...
    # ==========================================
    # 1. Aplica o filtro de Outliers primeiro
    ...
    # Se a lista tiver algo selecionado, aplica o filtro de canais múltiplos
    if len(canal_sel) > 0:
    ...
    # Dicionário rápido de Filial por Motorista para tooltips
    if not df_uni.empty:
    ...
    aba1, aba2, aba3, aba4, aba5, aba6, aba7, aba8 = st.tabs([
        ...
        "🔄 Recorrência", "🛣️ Rotas/Mapa", "📝 Tratativas", "🚨 Fraudes"
    ])
    ...
    # ABA 5: RECORRÊNCIA MENSAL REATIVA
    with aba5:
        st.subheader("🔄 Histórico Mensal de Ofensores (Filtrado)")
        if not df_uni.empty:
            ...
            st.markdown("**📋 Motoristas Reincidentes (Qtd de meses com ocorrência no filtro atual):**")
    ...
    # ==========================================
    # ✨ ABA 6: MAPA DE ROTAS (A VACINA FINAL CONTRA ERRO DE TEXTO VS NÚMERO)
    # ==========================================
    with aba6:
        ...
        if not df_rotas.empty:
            ...
            # 🛡️ MÁGICA DE PROTEÇÃO: Forçamos que TUDO relacionado a Rota seja Texto Puro
            ...
            if not df_mapa_plot.empty:
                st.plotly_chart(px.scatter_mapbox(df_mapa_plot, lat="LATITUDE", lon="LONGITUDE", size="Total_Geral", color="Total_Geral", zoom=7, mapbox_style="carto-positron"), use_container_width=True)
            else:
    ...
    with aba8:
        ...
        if not df_uni.empty:
            ...
            if not alertas.empty:
                # 🛡️ Proteção anti-duplicidade na hora de mostrar o Dossiê
                alertas = alertas.drop_duplicates(subset=['Pedido', 'Motivo'])
                alertas = alertas.loc[:, ~alertas.columns.duplicated()] # Remove colunas iguais se ocorrerem no merge
